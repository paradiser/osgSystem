...
*/
	int master_sockfd, server_or_client_sockfd;
	struct sockaddr_in master_address, server_or_client_address;
	...
	while(1) {
		...
		if(server_or_client_sockfd < 0){
			printf("server or client accept failed!\n");
			close(server_or_client_sockfd);
			return -1;
		}
